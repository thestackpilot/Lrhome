...
//dd($ship_to_content);
                                    ...
                                    'sections' => [
                                        ...
                                        [
                                            ...
                                            'content' => [
                                                ...
                                                'Discount' => number_format($transaction['Discount'], ConstantsController::ALLOWED_DECIMALS) . "%",
                                                ...
                                                'Other Charges' => ConstantsController::CURRENCY . number_format($transaction['OtherCharges'], ConstantsController::ALLOWED_DECIMALS),
                    ...
                    } else if ($transaction['TransactionType'] == 'Sales Invoice' || $transaction['TransactionType'] == 'One Step Invoice') {
                        foreach ($transaction['Details'] as $index => $view) {
                            $column = CommonController::get_selected_columns($view, [
                                'ImageName',
                                'ItemID',
                                    ...
                                    'sections' => [
                                        ...
                                        [
                                            ...
                                            'content' => [
                                                ...
                                                'Discount' => ($transaction['Discount'] == 'N/A' ? number_format("0.00", ConstantsController::ALLOWED_DECIMALS) . '%' : number_format((float) $transaction['Discount'], ConstantsController::ALLOWED_DECIMALS) . '%'),
    ...
    public function get_credit_memos(Request $request)
    {
        ...
        if (count($request->all()) > 0 && isset($request->submit)) {
            ...
            if (isset($memos['CreditMemos'])) {
                ...
                foreach ($memos['CreditMemos'] as $memo) {   //dd( $memo['Details'] );
                                ...
                                'sections' => [
                                    ...
                                    [
                                        ...
                                        'content' => [
                                            ...
                                            'Discount' => number_format($memo['Discount'], ConstantsController::ALLOWED_DECIMALS) . "%",
        ...
        if (count($request->all()) > 0 && isset($request->submit)) {
            ...
            if (isset($memos['DebitMemos'])) {
                ...
                if ($request->has('draw') && $request->draw) {
                    die(json_encode(
                        [
                            'recordsFiltered' => isset($memos["DebitMemos"]) && count($memos["DebitMemos"]) > 0 ? count($memos["DebitMemos"]) : 0, //$memos['TotalRows'],
                            'recordsTotal' => isset($memos["DebitMemos"]) && count($memos["DebitMemos"]) > 0 ? count($memos["DebitMemos"]) : 0, //$memos['TotalRows'],
                            'draw' => $request->draw + 1,
                            'data' => $table['tbody']
                        ]
                    ));
            ...
            $memos["TotalRows"] = isset($memos["DebitMemos"]) && count($memos["DebitMemos"]) > 0 ? count($memos["DebitMemos"]) : 0;
            ...
            if (isset($invoices['SalesInvoices'])) {
                ...
                foreach ($invoices['SalesInvoices'] as $invoice) {
                    ...
                    $bill_to_details = [
                        ...
                        'email' => $invoice['BillToAddress']['Email'] ? $invoice['BillToAddress']['Email'] : '',
                    ];
                    ...
                    $ship_to_details = [
                        ...
                        'email' => $invoice['ShipToAddress']['Email'] ? $invoice['ShipToAddress']['Email'] : '',
                    ];
                                ...
                                'sections' => [
                                    ...
                                    [
                                        ...
                                        'content' => [
                                            ...
                                            'Discount' => ($invoice['Discount'] == 'N/A' ? number_format("0.00", ConstantsController::ALLOWED_DECIMALS) . '%' : number_format($invoice['Discount'], ConstantsController::ALLOWED_DECIMALS) . '%'),
                    ...
                    if (isset($this->active_theme_json->theme_slug) && $this->active_theme_json->theme_slug === 'lr') {
                        foreach ($view_order['Detail'] as $index => $view) {
                            ...
                            $column['BackOrderQty'] = isset($view['BackOrder']) && CommonController::check_bit_field($view, 'BackOrder')
                                ? ((isset($view['ETADate']) ? 'ETA: ' . Carbon::parse($view['ETADate'])->format('M-d-Y') : '') . " <br>".
