@php
    ...
    if (isset($tabular) && $tabular == 'yes') {
        ...
        $tabs = ['All', 'Open', 'Back Orders', 'Completed'];
        /*
    ...
    function get_table($table, $tab = '')
    {
        ...
        if ($table['tbody']) {
            foreach ($table['tbody'] as $row) {
                if ($tab && isset($row['tab']) && $tab != $row['tab'] && $tab != 'All')
                    continue;
                $table_body .= '<tr>';
        ...
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            @foreach($tabs as $k => $tab)
                <li class="nav-item" role="presentation">
                    <button class="nav-link {{$k == 0 ? 'active' : ''}}" id="{{substr(hash('sha256', $tab), 0, 8)}}-tab"
                        data-bs-toggle="tab" data-bs-target="#{{substr(hash('sha256', $tab), 0, 8)}}" type="button" role="tab"
                        aria-controls="{{substr(hash('sha256', $tab), 0, 8)}}" aria-selected="true">{{$tab}}</button>
                </li>
            @endforeach
        </ul>
        <div class="tab-content" id="myTabContent">
            @foreach($tabs as $k => $tab)
                <div class="tab-pane fade show {{$k == 0 ? 'active' : ''}}" id="{{substr(hash('sha256', $tab), 0, 8)}}"
                    role="tabpanel" aria-labelledby="{{substr(hash('sha256', $tab), 0, 8)}}-tab">
                    {!!get_table($table, $tab)!!}
                </div>
            @endforeach
        </div>
...
<div class="modal fade other-detail-modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                {{-- <button type="button" class="btn btn-secondary close-modal other-detail-modal-close"
...
<div class="loader-container" id="loader-container"
    ...
    <div class="loader"
        style="border: 50px solid #f3f3f3; border-top: 50px solid #660000; border-radius: 50%; width: 100px; height: 100px; animation: spin 1s linear infinite;">
    </div>
</div>
@section('styles')
    @parent
                ...
                data.body.sections.forEach((section, i) => {
                    ...
                    if (Array.isArray(section.content) && typeof section.content.length !== 'undefined') {
                        ...
                        Object.keys(section.content.tabs).forEach(function (tab, i) {
                            ...
                            modal_body += `<button data-type="modal" class="nav-link ${i == 0 ? 'active' : ''}" id="${tab}-tab" data-bs-toggle="tab" data-bs-target="#${tab}" type="button" role="tab" aria-controls="${tab}" aria-selected="true">${title}</button>`;
                            modal_body += '</li>';
                        });
                        modal_body += '</ul>';
                                ...
                                if (index == 'ImageName' && row[index] !== '') {
                                    ...
                                    modal_body += "<td>" +
                                        (row[index] == null || row[index] == ''
                                            ...
                                            : (index == 'Price'
                                                ? '$' + parseFloat(row[index] ? row[index].toString().replace(',', '') : 0).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,') : row[index]) )
                                        + "</td>";
